-- Temporary dermatology and laser booking records, deliberately labelled pending approval.
-- See hostinger_care_departments_placeholder_seed.sql for the portable Hostinger version.

START TRANSACTION;

UPDATE `branch_specialties` SET `is_active` = true WHERE `department` IN ('dermatology', 'laser');
...
SELECT d.`id`, b.`id`, true FROM `dentists` d JOIN `branches` b ON b.`is_active` = true
...
SELECT d.`id`, s.`id`, true FROM `dentists` d JOIN `services` s ON s.`department` = d.`department` AND s.`is_active` = true
WHERE d.`name` IN ('فريق الجلدية والتجميل — قيد الاعتماد', 'فريق الليزر — قيد الاعتماد');
...
FROM `dentists` d JOIN (SELECT 0 AS `day_of_week` UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4) schedule
WHERE d.`name` IN ('فريق الجلدية والتجميل — قيد الاعتماد', 'فريق الليزر — قيد الاعتماد')
  AND NOT EXISTS (SELECT 1 FROM `working_hours` existing WHERE existing.`dentist_id` = d.`id` AND existing.`day_of_week` = schedule.`day_of_week` AND existing.`start_time` = '10:00:00' AND existing.`end_time` = '18:00:00');
